...
import { salesAnalysisPageTemplate } from "./salesAnalysisPageTemplate.js";

export default class HourlySales extends Program {
    constructor(windowContentElement, parentDataCallback, programConfig) {
        super();
        this.subscriberId = `HourlySales_${uuid()}`;
 ...
 async submitHandler(e) {
  ...
  render() {
    ...
    render(salesAnalysisPageTemplate(this.getHourlySalesArr(), this.weeklyTotal, controls), this.windowContentElement);
  }
  ...
  updateTotals() {
    Object.keys(this.hourlySalesTemplate).forEach((weekday) => {
      let total = Object.keys(this.hourlySalesTemplate[weekday].hours).reduce(
        (total, hour) =>
          (total += Number(this.hourlySalesTemplate[weekday].hours[hour])),0);
